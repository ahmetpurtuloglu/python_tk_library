...
class EditCategory(tk.Toplevel):
    def __init__(self, parent, rowid, gid, name):
        super().__init__()
    ...
    def update_values(self):
        try:
            self.database_lib.edit_category(gid=self.gid, name=self.name.get())
            self.parent.tv.item(self.rowid, values=(self.gid, self.name.get()))
            self.close_window()
